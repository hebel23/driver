// list_rwlock.c: konkurrierende Listenoperationen - RW-Lock
//
// Mechanismen:
...
static rwlock_t rcul_list_lock;
...
static unsigned long long rcul_cycle = 10ULL;
...
static void rcul_add_messwert (long wert)
{
	...
	write_lock_irqsave (&rcul_list_lock, flags);
	rcul_zaehler++;
	...
	write_unlock_irqrestore (&rcul_list_lock, flags);
}
...
static ssize_t messwert_show (struct device* dev, struct device_attribute* attr, char* buf)
{
	...
	read_lock_irqsave (&rcul_list_lock, flags);
	pos = buf;
	list_for_each_entry_safe (mw, n, &rcul_list_head, list)
	{
		...
		{
			read_unlock_irqrestore (&rcul_list_lock, flags);
			return strlen(buf);
		}
		pos = buf + strlen(buf);
	}
...
MODULE_DESCRIPTION("list_rwlock --- konkurrierende Listenoperationen mit RW-Lock");
