// list_rcu.c: konkurrierende Listenoperationen - RCU und Spinlock
//
// Mechanismen:
...
static spinlock_t rcul_list_lock;
...
static void rcul_add_messwert (long wert)
{
	...
	spin_lock_irqsave (&rcul_list_lock, flags);
	rcul_zaehler++;
	...
	list_add_tail_rcu (&mw->list, &rcul_list_head);

	spin_unlock_irqrestore (&rcul_list_lock, flags);
}
...
static ssize_t messwert_show (struct device* dev, struct device_attribute* attr, char* buf)
{
	...
	// Fehlerbeispiel: folgende Zeilen provozieren Deadlock
	spin_lock (&rcul_list_lock);
	spin_unlock (&rcul_list_lock);

	rcu_read_lock();
	pos = buf;
	list_for_each_entry_rcu (mw, &rcul_list_head, list)
	{
...
static ssize_t delete_store (struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
	...
	// Fehlerbeispiel: Spin-Lock hier anstatt in Schleife
	// spin_lock_irqsave (&rcul_list_lock, flags);
	for (i = 0; i < anzahl; i++)
	{
		spin_lock_irqsave (&rcul_list_lock, flags);
...
MODULE_DESCRIPTION("list_rcu --- konkurrierende Listenoperationen mit RCU und Spinlock");
