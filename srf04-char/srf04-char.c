...
static ssize_t srf04_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	...
	len = strlen(val);
	if ( !copy_to_user(buf, val, strlen(val))) {
		(*offset) += strlen(val);
		return strlen(val);
	}
	else
...
static int srf04_probe(struct platform_device *pdev)
{
	...
	if ((ret = register_chrdev_region(data->devt, 1, MODULE_NAME)) < 0) 
	{
...
static int srf04_remove(struct platform_device *pdev)
{
	...
	dev_info(data->dev, "srf04_remove\n");
